...
import { Brain, Briefcase, CalendarDays, Check, ChefHat, ChevronLeft, ChevronRight, Code2, Coffee, Dumbbell, Heart, ListChecks, LogOut, Music, Pencil, PersonStanding, Plus, Star, Trash2, Utensils, X } from "lucide-react";
...
export default function Dashboard() {
  ...
  return <main className="schedule-page"><div className="schedule-card">
    <header className="schedule-header"><div className="profile-summary" aria-label="Signed-in user"><span className="profile-avatar">{user?.name?.trim()?.charAt(0)?.toUpperCase() || user?.email?.charAt(0)?.toUpperCase() || "U"}</span><span className="profile-copy"><b>{user?.name || "Loading…"}</b><small>{user?.email || ""}</small></span></div><h1>Daily Schedule Routine Track</h1><button className="logout-button" onClick={logout} suppressHydrationWarning><LogOut size={17} />Logout</button></header>
    <nav className="schedule-nav" aria-label="Month navigation"><button className="previous-month" onClick={() => changeMonth(-1)} disabled={isLoading} suppressHydrationWarning><ChevronLeft />Previous</button><button className="current-month" onClick={() => setMonth(localMonth())} disabled={isLoading} suppressHydrationWarning><CalendarDays size={20} />{monthFormatter.format(new Date(year, monthNumber - 1, 1))}</button><button className="next-month" onClick={() => changeMonth(1)} disabled={isLoading} suppressHydrationWarning>Next<ChevronRight /></button>{canEdit && (planned ? <><button className="manage-routines" onClick={() => setManageOpen(true)}><ListChecks size={19} />Manage</button><button className="add-routine" onClick={() => resetForm()}><Plus size={20} />Add routine</button></> : <button className="create-plan" onClick={createPlan} suppressHydrationWarning><Plus size={19} />Create month plan</button>)}</nav>
    ...
    <section className="mobile-agenda" aria-label="Daily routine agenda">
      ...
      <div className="agenda-date-nav"><button onClick={() => changeSelectedDay(-1)} disabled={selectedDate === `${month}-01`} aria-label="Previous day" suppressHydrationWarning><ChevronLeft /></button><label><CalendarDays size={17} /><input type="date" value={selectedDate} min={`${month}-01`} max={`${month}-${String(daysInMonth).padStart(2, "0")}`} onChange={event => setSelectedDate(event.target.value)} aria-label="Choose agenda date" suppressHydrationWarning /></label><button onClick={() => changeSelectedDay(1)} disabled={selectedDate === `${month}-${String(daysInMonth).padStart(2, "0")}`} aria-label="Next day" suppressHydrationWarning><ChevronRight /></button></div>
    ...
    <footer className="mobile-footer"><span>© 2026 Biswojit</span><span>Routine Flow</span></footer>
    <div className={`tracker-wrap ${isLoading ? "is-loading" : ""}`}><section className="tracker" style={{ gridTemplateColumns: `var(--date-column) repeat(${Math.max(routines.length, 1)}, minmax(var(--routine-column), 1fr))` }}>
      ...
      {routines.length ? routines.map(routine => <div className="tracker-head routine-head" style={{ "--routine": routine.color }} key={routine.id}><RoutineIcon name={routine.icon} /><span title={routine.title}>{routine.title}</span></div>) : <div className="tracker-head routine-head">{planned ? "Add your first routine" : "No routines"}</div>}
  ...
  {manageOpen && <div className="overlay"><section className="manage-modal" role="dialog" aria-modal="true" aria-labelledby="manage-title"><button className="close" onClick={() => setManageOpen(false)} aria-label="Close routine manager"><X /></button><div className="manage-title"><span><ListChecks /></span><div><p>MONTH ROUTINES</p><h2 id="manage-title">Manage routines</h2><small>{monthFormatter.format(new Date(year, monthNumber - 1, 1))}</small></div></div><div className="manage-list">{routines.length ? routines.map(routine => <article className="manage-item" style={{ "--routine": routine.color }} key={routine.id}><span className="manage-icon"><RoutineIcon name={routine.icon} /></span><div><b>{routine.title}</b><small>{routine.frequency.replace("-", " ")}</small></div><div className="manage-actions"><button className="manage-edit" onClick={() => { setManageOpen(false); resetForm(routine); }} aria-label={`Edit ${routine.title}`}><Pencil size={16} /><span>Edit</span></button><button className="manage-delete" onClick={() => { setManageOpen(false); setRemoveTarget(routine); }} aria-label={`Delete ${routine.title}`}><Trash2 size={16} /><span>Delete</span></button></div></article>) : <div className="manage-empty">No routines to manage for this month.</div>}</div><button className="manage-add" onClick={() => { setManageOpen(false); resetForm(); }}><Plus size={18} />Add new routine</button></section></div>}
